...
<body>
      ...
      <section class="panel">
        <div class="workspace-header">
          <div>
            ...
            <h2 id="headline" class="headline">Lanzar una nueva investigacion</h2>
            <p id="workspace-copy" class="workspace-copy">Elegi tu modelo local, escribi la consigna y la consola va a mostrar cada etapa: planning, busqueda, extraccion, reflexion, review humana y sintesis final.</p>
          </div>
          <div class="pill-row">
        ...
        <div id="approval-box" class="approval-box hidden">
          ...
          <h3 id="approval-title">Esperando decision</h3>
          <p id="approval-prompt" class="subtle"></p>
          <textarea id="approval-summary" placeholder="Opcional: agrega aclaraciones o feedback para el agente."></textarea>
  ...
  <script>
    ...
    function upsertRun(run) {
      const index = state.runs.findIndex((candidate) => candidate.thread_id === run.thread_id);
      if (index >= 0) {
    ...
    function renderRuns(runs) {
      state.runs = runs;
      if (!runs.length) {
        elements.runList.innerHTML = '<div class="empty-state">Todavia no hay corridas registradas. Inicia una nueva desde el panel principal.</div>';
        return;
      }
    ...
    function renderStats(run) {
      ...
      const stats = [
        ...
        { label: 'Iteracion', value: stateSnapshot.iteration_count || 0 },
      ];
    ...
    function pushLiveEvent(threadId, entry) {
      if (!state.liveEventsByThread[threadId]) {
        state.liveEventsByThread[threadId] = [];
      }
      state.liveEventsByThread[threadId].unshift(entry);
      state.liveEventsByThread[threadId] = state.liveEventsByThread[threadId].slice(0, 40);
    }

    function renderLiveFeed(threadId) {
      const entries = state.liveEventsByThread[threadId] || [];
      if (!entries.length) {
        elements.feedList.innerHTML = '<div class="empty-state">Cuando el modelo devuelva reasoning o previews, van a aparecer aca por SSE.</div>';
        return;
      }
      elements.feedList.innerHTML = entries.map((entry) => `
        <article class="feed-item">
          <div class="meta-row">
            <span class="pill">${escapeHtml(entry.label)}</span>
            <span class="mono">${escapeHtml(entry.phase || '')}</span>
          </div>
          <p class="subtle" style="margin-top:8px">${escapeHtml(entry.content || entry.message || '')}</p>
        </article>
      `).join('');
    }
    ...
    function renderTasks(run) {
      ...
      if (!tasks.length) {
        elements.tasksList.innerHTML = '<div class="empty-state">Todavia no hay tareas planificadas.</div>';
        return;
      }
    ...
    function renderSources(run) {
      ...
      if (!sources.length) {
        elements.sourcesList.innerHTML = '<div class="empty-state">Las fuentes aparecen despues de la fase de busqueda.</div>';
        return;
      }
    ...
    function renderRun(run) {
      ...
      elements.workspaceCopy.textContent = run.last_message || 'La corrida esta activa.';
      elements.threadPill.textContent = run.thread_id;
      setStatus(run.status);
      renderStats(run);
      renderTimeline(run);
      renderLiveFeed(run.thread_id);
      renderTasks(run);
      renderSources(run);
      renderReport(run);
      renderApproval(run);
    ...
    async function selectRun(threadId, connectStream = true) {
    ...
    function startEventStream(threadId) {
      if (state.eventSource) {
        state.eventSource.close();
      }
      const source = new EventSource(`/api/runs/${threadId}/events`);
      ...
      source.addEventListener('llm_stage_started', (event) => {
        const payload = JSON.parse(event.data);
        pushLiveEvent(threadId, {
          label: 'LLM Stage',
          phase: payload.phase,
          message: `${payload.model} inicio ${payload.phase}`,
        });
        renderLiveFeed(threadId);
      });

      source.addEventListener('llm_reasoning', (event) => {
        const payload = JSON.parse(event.data);
        pushLiveEvent(threadId, {
          label: 'Thinking',
          phase: payload.phase,
      ...
      source.addEventListener('llm_output_preview', (event) => {
        const payload = JSON.parse(event.data);
        pushLiveEvent(threadId, {
          label: 'Preview',
          phase: payload.phase,
      ...
      source.addEventListener('run_task_failed', (event) => {
        const payload = JSON.parse(event.data);
        pushLiveEvent(threadId, {
          label: 'Runtime',
    ...
    async function submitRun(event) {
      ...
      state.liveEventsByThread[created.thread_id] = [];
      upsertRun(created);
      renderRun(created);
      renderRuns(state.runs);
      startEventStream(created.thread_id);
    }
    ...
    async function submitDecision(decision) {
      ...
      renderRun(run);
      renderRuns(state.runs);
      startEventStream(run.thread_id);
    }
